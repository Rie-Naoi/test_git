...
- 注釈付き（annotated）版・・・名前、コメント、署名
- 軽量（lightweight）版・・・名前だけ

### タグを作成する（注釈付きタグ）

```bash
#-aオプションをつけると注釈付きタグが作成できる
...
タグをリモートに送信するにはgit push コマンドで別途指定する  
（git pushやgit push origin masterではタグはアップされない）  
...
```bash
#タグを一斉に送信する
#--tagsをつけるとローカルにあってリモートリポジトリにないタグを一斉に送信する
